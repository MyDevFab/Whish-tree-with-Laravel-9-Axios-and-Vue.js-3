...
<body>
  <div class='container torn_container torn_left torn_right' id="torn_edge_banner">
    ...
    <div class="" id="tv">
      ...
      <h3>Theme: {{resolutionCategory}}</h3>
      <i style="cursor: pointer; color: 'black' !important; z-index: 1;" @click="deleteResolution(resolution_id)" class="bi bi-trash"></i>
      <!-- <i  @click="toCheckResolution(resolution_id)" style="cursor: pointer;"  class="bi bi-circle"></i> -->
      <!-- <i v-if="resolution_checked == 1"  class="bi bi-check2-circle"></i> -->
    </div>
      <button class="btn-step" @click="toggleModal">
           Add a Step +    
      </button>
        <CreateTodo v-if="todoModal" :resolution_id="resolution_id" :resolution_title="resolution_title" :resolution_deadline="resolution_deadline" :resolution_category="resolution_category" @closeModal="toggleModal" @NewTodo="newTodo"/>
    ...
    <div style="display: flex;
      ...
      <i v-if="todo.checked == 0" @click="toCheckTodo(todo.id)" style="cursor: pointer;" class="bi bi-circle"></i>
      <i v-if="todo.checked == 1" @click="toUncheckTodo(todo.id)" style="cursor: pointer;" class="bi bi-check2-circle" ></i>

      <label  class="form-check-label" for="flexCheckDefault" :style="todo.checked == 1 ? changeStyle : null" >
        {{ todo.content }}&nbsp;&nbsp;&nbsp;<i style="cursor: pointer;" @click="deleteTodo(todo.id)" class="bi1 bi-trash"></i> 
      </label>
    </div>
      <div><label class="hidden form-check-label" for="flexCheckDefault" style="display: flex; flex-wrap: wrap;">
        nnnnnnnnnnnnnnn nnnnnnnnnnnnnnnnnnnnn nnnnnnnnnnnnnnnnn 
      </label></div>
    <div><button type="submit" @click="toCheckResolution(resolution_id)" class="btn-victory glow-on-hover" title="Click here to mark your resolution completed. Congrats!">I made it!</button>&nbsp;&nbsp;&nbsp;
    <button type="submit" @click="onholdResolution(resolution_id)" class="btn-victory2" title="Click here to keep this resolution for another tree.">For later</button>
    </div>
...
export default {
  name: "show-todos",
  props: ["id", "resolution_id", "resolution_title", "resolution_deadline", "resolution_category", "resolution_checked"],
  components: {
    CreateTodo
  },
  data() {
  ...
  methods: {
    ...
    async currentDeadline(id) {
      ...
      .then((response) => {
        ...
        axios.get(`http://localhost:8000/api/categories/${response.data[0].category_id}`).then((response) => {
        this.resolutionCategory = response.data[0].name
      })
      }).catch((error) => {
        console.log(error);
      });
    ...
    toCheckTodo(id){
      console.log('clicked')
      axios.put(`http://localhost:8000/api/todos/status/${id}`, {
    ...
    toCheckResolution(id){
      console.log('clicked')
      axios.put(`http://localhost:8000/api/resolutions/status/${id}`, {
        checked : !this.checkResolution
      })
      this.$router.push(`/tree/${this.id}`)
    },
      onholdResolution(id){
      axios.put(`http://localhost:8000/api/resolutions/onhold/${id}`, {
        onhold : 1
      }) 
        // this.$router.push(`/tree/${this.id}`) 
    },
...
.reso-checked {
  ...
  --torn-background-color: orange;
}

.step-checked {

  --torn-background-color: gold;
}
...
--torn-shadow-offset-x:-2px;
	...
	--torn-background-color:rgb(246, 240, 226);
...
.glow-on-hover:before {
    content: '';
    background: linear-gradient(45deg, #ffa600, #efb483, #fffb00, #48ff00, #00ffd5, #00ffbb, #bbff00, #00ffb3, #ffae00);
    position: absolute;
    top: -2px;
    left:-2px;
    background-size: 400%;
